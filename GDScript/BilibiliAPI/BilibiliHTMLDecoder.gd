class_name BilibiliHTMLDecoder

static var _named_entities := {
	"&amp;": "&", "&lt;": "<", "&gt;": ">", "&quot;": '"', "&apos;": "'", "&nbsp;": " ",
	...
	"&piv;": "ϖ", "&bull;": "•", "&hellip;": "…", "&prime;": "′", "&Prime;": "″",
	"&oline;": "‾", "&frasl;": "⁄", "&weierp;": "℘", "&image;": "ℑ", "&real;": "ℜ",
...
static func decode(text: String) -> String:
	...
	dec_regex.compile("&#(\\d+);")
	for match in dec_regex.search_all(result):
		var code = match.get_string(1).to_int()
		if code > 0:
			result = result.left(match.get_start(0)) + char(code) + result.right(-match.get_end(0))

	var hex_regex = RegEx.new()
	hex_regex.compile("&#x([0-9a-fA-F]+);")
	for match in hex_regex.search_all(result):
